...
app.post("/create-order", (req, res) => {
  try {
    const cart = req.body.cart; // ✅ DEFINE IT HERE

    if (!cart) {
      return res.status(400).json({ error: "Cart is missing" });
    }
...
258deg, #000000, #00000094);
        padding: 10px 20px;
        font-weight: bolder;
        height: max-content;
        border-radius: 6px;
        border: none;
        box-shadow: 0 0 12px 0px darkslategrey;
      }
      a {
        text-decoration: none;
      }
  ...
  html += `<a href="ecrinamina.onrender.com" target="_blank" rel="noopener noreferrer">
    <button class="back">
      Retour au Site 
    </button>
  </a>`;
    ...
    html += `
      ...
      <div class="cart-item">
        ...
        <div class="cart-item-details">
          ...
          <strong style="color: crimson;">lieu de livraison:${item.localisation}</strong>
        </div>
      </div>
    </a>
    `;
